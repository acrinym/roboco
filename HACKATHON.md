...
- **Database**: migrations `099_modelprovider_nebius` (enum value added inside an `autocommit_block`) and `100_seed_nebius_provider` (a key-gated, `enabled=false` provider seed; numbered after slave's ZAI pair took 097/098 in the merge).
...
The picker (Settings -> AI Routing -> Nebius default model) can still narrow or change the choice within the Nemotron family and the rest of the catalog, but operator inaction always lands on Nemotron. The shipped default id is live-verified against the Token Factory catalog: the originally drafted `nvidia/nemotron-3-super-120b` does not exist upstream, and the real id (`nvidia/nemotron-3-super-120b-a12b`) was confirmed with a live `GET /v1/models` before shipping. The four live-verified Nemotron ids are static catalog entries, so Nemotron is configurable at every routing level (Mix-mode per-agent pins and complexity overrides included), not just as the global default.
...
### 5. v0.31.0: the hackathon stack ships in the released product (2026-09-17)

Everything above stopped being submission-branch work on 2026-09-17: PR #1103 (the Nebius Token Factory provider, Nemotron levels parity, and the `run_sandbox_tests` Sandboxes verb) merged to the mainline, and v0.31.0 released it on `master`, tagged and published, with the release workflow building the `roboco-agent-nebius` image and every compose image set listing it. A judge running `make quickstart` on the released tag gets the Nebius integration, not a checkout of a feature branch.

The same release cycle stress-tested the Sandboxes client for real: with beta Sandboxes access granted, the full flow (archive upload, instance spawn, extraction, command execution, result polling) ran green end to end and metered real cost, and it caught one genuine integration bug (the host-rooted operation `Location` header double-pathing poll requests into empty 404s) that is fixed and regression-tested in the shipped code.
