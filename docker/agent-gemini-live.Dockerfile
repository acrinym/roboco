# gemini LIVE-CHAT agent - interactive intake/secretary session on gemini.
# =============================================================================
...
# extension for hummin). Builds on the gemini runtime image (CLI + roboco venv).
# Both roles ride this ONE image - ROBOCO_AGENT_ROLE selects the behavior.
# =============================================================================

FROM roboco-agent-gemini

LABEL role="gemini-live"
LABEL description="gemini interactive live chat (intake + secretary, generic driver)"

# The in-container receiver the orchestrator delivers turns to.
EXPOSE 9000

COPY --chmod=755 docker/scripts/entrypoints/gemini-live-entrypoint.sh /app/scripts/entrypoints/gemini-live-entrypoint.sh

# Override the one-shot entrypoint: render config, then the generic driver.
ENTRYPOINT ["/app/scripts/entrypoints/gemini-live-entrypoint.sh"]
