# codex LIVE-CHAT agent - interactive intake/secretary session on codex.
# =============================================================================
...
# extension for hummin). Builds on the codex runtime image (CLI + roboco venv).
# Both roles ride this ONE image - ROBOCO_AGENT_ROLE selects the behavior.
# =============================================================================

FROM roboco-agent-codex

LABEL role="codex-live"
LABEL description="codex interactive live chat (intake + secretary, generic driver)"

# The in-container receiver the orchestrator delivers turns to.
EXPOSE 9000

COPY --chmod=755 docker/scripts/entrypoints/codex-live-entrypoint.sh /app/scripts/entrypoints/codex-live-entrypoint.sh
