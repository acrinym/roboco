# openrouter LIVE-CHAT agent - interactive intake/secretary session on openrouter.
# =============================================================================
...
# extension for hummin). Builds on the openrouter runtime image (CLI + roboco venv).
# Both roles ride this ONE image - ROBOCO_AGENT_ROLE selects the behavior.
# =============================================================================

FROM roboco-agent-openrouter

LABEL role="openrouter-live"
LABEL description="openrouter interactive live chat (intake + secretary, generic driver)"

# The in-container receiver the orchestrator delivers turns to.
EXPOSE 9000

COPY --chmod=755 docker/scripts/entrypoints/openrouter-live-entrypoint.sh /app/scripts/entrypoints/openrouter-live-entrypoint.sh
